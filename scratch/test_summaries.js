...
async function verify() {
  ...
  try {
    ...
    const pid = '1282332';
    console.log(`\n--- Testing Summary Endpoints for Project ID: ${pid} ---`);
    
    const summaryEndpoints = [
        '/api/services/app/Project/GetProjectSummaryByProjectId',
        '/api/services/app/Project/GetProjectSummaryDtoById',
        '/api/services/app/Project/GetProjectSummaryByIdForMob'
    ];
    ...
    for (const endpoint of summaryEndpoints) {
        try {
            ...
            const res = await axios.get(`${baseUrl}${endpoint}`, {
                headers,
                params: { id: pid, projectId: pid } // Try both
            });
            console.log('SUCCESS');
            console.log(JSON.stringify(res.data.result, null, 2).substring(0, 1000));
        } catch (e) {
