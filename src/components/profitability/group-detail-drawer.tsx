...
import { X, Clock, FileText, CheckCircle2, TrendingUp, TrendingDown, Layers, Target, CheckSquare, DollarSign, Briefcase } from "lucide-react";
import { cn } from "@/lib/utils";
import { ProfitabilityGroup } from "./profitability-table";
import { getLiveProjectDetails } from "@/app/actions/profitability";
import { generateGroupNarrative } from "@/app/actions/ai-insights";
...
export function GroupDetailDrawer({ group, isOpen, onClose }: GroupDetailDrawerProps) {
  ...
  useEffect(() => {
    if (!isOpen || !group.key) return;
    
    let isMounted = true;
    setIsLoading(true);
    setError(null);

    const projectIds = group.projects.map(p => p.workguruId).filter(Boolean) as string[];

    Promise.all(projectIds.map(id => getLiveProjectDetails(id)))
      .then((results) => {
        if (!isMounted) return;
        ...
        for (const res of results) {
          if (res.success && res.data) {
            totalActualHours += res.data.totalActualHours || 0;
            totalBudgetHours += res.data.totalBudgetHours || 0;
            completedTasks += res.data.completedTasks || 0;
            totalTasks += res.data.totalTasks || 0;
            wipValue += res.data.wipValue || 0;
          } else {
            hasError = true;
          }
        }
        ...
        if (hasError && results.length === 0) {
           setError("Failed to load live project details for some projects.");
        } else {
           setLiveData({
             totalActualHours,
             totalBudgetHours,
  ...
  const pInvoiced = group.totalInvoiced;
  const pCost = group.totalCost;
  const gp = pInvoiced - pCost;
  const wgEstimatedProfit = group.totalEstimatedInvoiced - group.totalEstimatedCost;
  const variance = gp - wgEstimatedProfit;
  const marginPct = getGPMarginPct(pInvoiced, pCost);

  const pMat = group.totalMaterials;
  const pLab = group.totalLabour;
  const pEstMat = group.totalEstimatedMaterials;
  const pEstLab = group.totalEstimatedLabour;
  const pEstCost = group.totalEstimatedCost;
  const pEstInvoiced = group.totalEstimatedInvoiced;

  // Calculate insights
  const insights = React.useMemo(() => {
    // For groups, "near complete" is tricky. Let's just say true if all projects are near complete, or just don't override.
    return generateProjectInsights({
      ...
      isNearCompleteOverride: false
    });
  }, [gp, pInvoiced, pCost, pEstInvoiced, pEstCost, pMat, pEstMat, pLab, pEstLab, liveData]);
  ...
  useEffect(() => {
    if (!isOpen || !group.key || !liveData) return;

    let isMounted = true;
    setIsNarrativeLoading(true);
    setNarrativeError(null);

    const subProjects = group.projects.map(p => ({
      number: p.projectNumber,
      gp: (p.invoicedAmount || 0) - (p.totalCost || 0),
      margin: getGPMarginPct(p.invoicedAmount || 0, p.totalCost || 0).toFixed(1) + "%",
      labourDiff: (p.labourCost || 0) - (p.estimatedLabourCost || 0),
      materialsDiff: ((p.materialsCost || 0) + (p.purchasesCost || 0)) - (p.estimatedMaterialsCost || 0)
    }));
    ...
    generateGroupNarrative(context)
      .then(res => {
        if (!isMounted) return;
    ...
    return () => { isMounted = false; };
  }, [isOpen, group.key, liveData, insights, gp, pEstInvoiced, pEstCost, variance, marginPct, pInvoiced, pMat, pEstMat, pLab, pEstLab]);
  
  const uniqueClients = Array.from(new Set(group.projects.map(p => p.clientName).filter(Boolean))).join(", ") || 'Unknown';
        ...
        <div className="flex-none p-5 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-3">
          <div className="flex items-start justify-between">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className="text-lg font-black text-slate-900 dark:text-white">Group {group.key}</span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider text-slate-600 bg-slate-100 border-slate-200 dark:text-slate-300 dark:bg-slate-800 dark:border-slate-700">
          ...
          <div className="flex items-center justify-between text-xs mt-1">
            <div className="flex flex-col">
              ...
              <span className="font-semibold text-slate-700 dark:text-slate-300">{uniqueClients}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              <Briefcase className="h-4 w-4" /> Financial Overview (Group)
            </h3>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col">
                <span className="text-xs text-slate-500 font-medium">Summed GP</span>
                <span className="text-lg font-bold text-slate-700 dark:text-slate-300">{formatCurrency(gp)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-slate-500 font-medium">Summed WG Est Profit</span>
              ...
              <div className="flex flex-col pt-3 border-t border-slate-100 dark:border-slate-800">
                <span className="text-xs text-slate-500 font-medium">Combined Variance</span>
              ...
              <div className="flex flex-col pt-3 border-t border-slate-100 dark:border-slate-800">
                <span className="text-xs text-slate-500 font-medium">Blended Margin</span>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              <DollarSign className="h-4 w-4" /> Invoiced vs Cost (Group)
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-1">
                <div className="flex justify-between items-end mb-1">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Summed Invoiced</span>
                  <span className="text-lg font-black text-slate-900 dark:text-white">{formatCurrency(pInvoiced)}</span>
                </div>
                <div className="flex justify-between items-center text-xs text-slate-500">
                  <span>Summed Est Invoiced</span>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex justify-between items-end mb-1">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Summed Cost</span>
                  <span className="text-lg font-black text-slate-900 dark:text-white">{formatCurrency(pCost)}</span>
                </div>
                <div className="flex justify-between items-center text-xs text-slate-500">
                  <span>Summed Est Cost</span>
          ...
          {/* Section 3: Cost Split */}
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              <Layers className="h-4 w-4" /> Cost Split (Group)
            </h3>
          ...
          {error && (
            <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-xl p-4 text-sm text-red-600 dark:text-red-400">
              <p className="font-bold flex items-center gap-2"><X className="h-4 w-4" /> Failed to load live data for group</p>
          ...
          {!isLoading && !error && liveData && (
            <>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                  <Clock className="h-4 w-4" /> Hours Tracking (Group)
                </h3>
                
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between text-sm">
                    <span className="font-medium text-slate-600 dark:text-slate-300">
                      Summed Actual: {liveData.totalActualHours.toFixed(1)}h
                    </span>
                    <span className="font-medium text-slate-600 dark:text-slate-300">
                      Summed Budget: {liveData.totalBudgetHours.toFixed(1)}h
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm flex flex-col justify-between">
                  <div className="flex items-center gap-2 text-slate-400 mb-2">
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider">Summed WIP Value</span>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm mt-8">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              <Target className="h-4 w-4" /> Group Insights
            </h3>
              ...
              {sortedInsights.length === 0 ? (
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-100 dark:border-slate-700 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">Group metrics are within normal expectations. No notable flags to report.</p>
                  </div>
                </div>
              ) : (
