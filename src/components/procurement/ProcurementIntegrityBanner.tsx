...
export function ProcurementIntegrityBanner({ integrity, syncHealth, isSyncing }: ProcurementIntegrityBannerProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 py-2 px-4 bg-slate-50 dark:bg-slate-900/30 border border-slate-200/60 dark:border-slate-800/60 rounded-xl mb-4">
      ...
      <div className="flex items-center gap-2 border-r border-slate-200 dark:border-slate-800 pr-6 h-6">
        <h3 className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Procurement Data Status</h3>
        <span className={cn(
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-tight">Details Loaded</span>
        </div>

        <Tooltip 
            content={
                <div className="text-left">
                    <p className="font-bold mb-1">Waiting for details</p>
        ...
        >
            <div className="flex items-center gap-1.5 cursor-help group">
              ...
              <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-tight group-hover:text-slate-700 dark:group-hover:text-slate-300">Waiting for details</span>
        ...
        {integrity.failedCount > 0 && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-tight">Failed To Load Details</span>
          </div>
        )}
      </div>
      ...
      <div className="ml-auto flex items-center gap-4">
        {syncHealth.retryQueueCount > 0 && (
          <div className="flex items-center gap-2 bg-blue-500/5 px-2 py-1 rounded-lg border border-blue-500/10">
            <RefreshCw className={cn("h-3 w-3 text-blue-500", isSyncing && "animate-spin-slow")} />
            <span className="text-[10px] font-bold text-blue-600 uppercase tracking-tight">
              {syncHealth.retryQueueCount} Waiting for retry
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5 text-slate-400">
          ...
          <span className="text-[10px] font-medium tracking-tight">
            {isSyncing ? "Syncing..." : loadingPercent === 100 ? "Sync Verified" : "Some PO details still missing"}
          </span>
        </div>
      </div>
    </div>
